...
class App extends Component {
  constructor(props){
    ...
    chart.forEach(item => { 
      const BookId = item.bookLikedID; 

      if (this.UserByBook[BookId]){ 
        this.UserByBook[BookId].push(item.peepsID);
      } else { 
        this.UserByBook[BookId] = [item.peepsID];
      }
    });
  }
